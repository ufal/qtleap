...
if ($input_file1 =~ /^(runs.*)output.txt/ ){
    $base1 = $1;
    ($base2) = ($input_file2 =~ /^(runs.*)output.txt/);
}
...
while(<$F1>){
    ...
    if ($id1 =~ /^(.*)\(([^)]*)\)/){
        ...
        my ($file2) = ($id2 =~ /\(([^)]*)\)/);
        $id1 = "$id\n" .
            c($MARKUP{'tred_open'}->[$HTML])."ttred $file1 & " . c($MARKUP{'tred_close'}->[$HTML]). "\n".
            c($MARKUP{'tred_open'}->[$HTML])."ttred $file2 & " . c($MARKUP{'tred_close'}->[$HTML]). "\n";
    }
