...
sub BUILD {
    ...
    while (my $document = $treex_reader->next_document()) {
        ...
        foreach my $bundle ($document->get_bundles()) {
            ...
            my @children = $src_ttree->get_children();
            foreach my $child (@children){
                print STDERR $child->t_lemma . "\n";
            }        
...
sub process_tnode {
    ...
    log_warn("...........to translate" . $trg_tnode->t_lemma_origin . "\n");
    return if $trg_tnode->t_lemma_origin !~ /^(clone|lookup)/;
    log_warn("not translated" .$trg_tnode->t_lemma_origin . "\n");
